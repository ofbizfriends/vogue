...
<#if forumMessages?has_content && forumMessages?size gt 0>
    ...
    <div class="product-prevnext">
      ...
      <b>
        <#if (viewIndex?int >0)>
          <a href="<@ofbizUrl>showforum?forumId=${parameters.forumId}&amp;VIEW_SIZE=${viewSize}&amp;VIEW_INDEX=${viewIndex?int-1}</@ofbizUrl>" class="btn btn-link">${uiLabelMap.CommonPrevious}</a> |
        </#if>
        <#if (listSize?int > 0)>
          <span>${lowIndex} - ${highIndex} ${uiLabelMap.CommonOf} ${listSize}</span>
        </#if>
        <#if highIndex?int < listSize?int>
          | <a href="<@ofbizUrl>showforum?forumId=${parameters.forumId}&amp;VIEW_SIZE=${viewSize}&amp;VIEW_INDEX=${viewIndex?int+1}</@ofbizUrl>" class="btn btn-link">${uiLabelMap.CommonNext}</a>
        </#if>
      </b>
   </div>
</#if>
